...
def generate_instace(items,capacity,min_capacity,max_capacity,min_value,max_value): # Create the instance on a .txt file
	...
	for i in range(0,n): # Fills the file with random numbers
		for j in range(0,2):
			if(j==0):
				value = str(random.randint(min_value+1,max_value+1)) # Generates random Vi
			else:
...
for i in range(0,m): # Loop to solve all the instances with the data given
	...
	print("----------------------------------------------------------------------------------")	# Esthetic
	file = open("pelos"+str(i+1)+".txt",'w') # Create a file where to save information
	generate_instace(n,w,w_min,w_max,v_min,v_max) # Once created, fills it with random instance
